...
void *thread_fn(void *socketNew){
    ...
    if(temp!=NULL){
        ...
        printf("Data Rescieved from Cache\n");
        printf("%s\n\n",response);
    }
        ...
        if(ParsedRequest_parse(request,buffer,len)<0){
            ...
            if(!strcmp(request->method,"GET")){
                ...
                printf("This code does'nt support any method apart from GET\n");
            }
        }
    }
    else if(bytes_send_client==0){
        printf("cliend is disconnected");
    }
...
int main(int argc , char* argv[]){
    ...
    printf("Binding on portnumber :%d",port_number);
